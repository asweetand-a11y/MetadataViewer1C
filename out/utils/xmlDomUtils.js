...
exports.applyFormChangesToXmlStringWithDom = exports.applyChangesToXmlStringWithDom = exports.formatXml = void 0;
 ...
 */
function applyChangesToXmlStringWithDom(originalXml, obj, xmlObjectType) {
...
exports.applyChangesToXmlStringWithDom = applyChangesToXmlStringWithDom;
/**
 * Применяет изменения к Properties используя DOM
 */
function applyPropertiesChangesWithDom(objElement, properties) {
    ...
    for (const [key, value] of Object.entries(properties)) {
        ...
        if (key === 'StandardAttributes' && Array.isArray(value)) {
            ...
            const findExistingStandardAttribute = (name) => {
                const list = standardAttrsElement.getElementsByTagNameNS
                    ? standardAttrsElement.getElementsByTagNameNS(XR_NS, 'StandardAttribute')
            ...
            const findDirectChildXr = (parent, localName) => {
            ...
            for (const attrRaw of value) {
                ...
                const attr = attrRaw['xr:StandardAttribute'] || attrRaw.StandardAttribute || attrRaw;
                const attrName = String(attr.name ?? attr.Name ?? attr['@name'] ?? '').trim();
                if (!attrName)
                    continue;
        ...
        if (key === 'RegisterRecords' && Array.isArray(value)) {
            ...
            for (const item of value) {
                if (typeof item === 'object' && item !== null) {
                    const itemElement = objElement.ownerDocument.createElementNS('http://v8.1c.ru/8.3/xcf/readable', 'xr:Item');
                    ...
                    if (typeof itemData === 'object' && itemData !== null) {
                        ...
                        itemElement.setAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'xsi:type', itemType);
                    ...
                    const itemElement = objElement.ownerDocument.createElementNS('http://v8.1c.ru/8.3/xcf/readable', 'xr:Item');
                    itemElement.setAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'xsi:type', 'xr:MDObjectRef');
 ...
 */
function setPropertyValueWithDom(element, value) {
    ...
    if (element.tagName === 'Type') {
        ...
        if (value && typeof value === 'object') {
            // Определяемый тип
            const typeSetValue = value['v8:TypeSet'] ?? value.TypeSet;
            if (typeSetValue !== undefined) {
                appendV8('TypeSet', asText(typeSetValue));
                return;
            }
            // Обычный/составной тип
            const typesValue = value['v8:Type'] ?? value.Type;
    ...
    if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
        ...
        const onlyTypeAndText = keys.every(k => k === 'type' || k === 'text' || k === '#text') ||
        ...
        if (hasType && onlyTypeAndText) {
            const typeVal = typeof vAny.type === 'object' && vAny.type !== null && '#text' in vAny.type
                ...
                : String(vAny.type);
            element.setAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'xsi:type', typeVal);
            if (hasText) {
    ...
    if (typeof value === 'string') {
        ...
        for (const item of value) {
            if (typeof item === 'object' && item !== null) {
                ...
                if ('Item' in item || 'xr:Item' in item) {
                    ...
                    const itemElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.3/xcf/readable', 'xr:Item');
                    const itemData = item.Item || item['xr:Item'];
                    let itemType = 'xr:MDObjectRef'; // Значение по умолчанию
                    let itemText = '';
                    if (typeof itemData === 'object' && itemData !== null) {
                    ...
                    const childElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', 'v8:item');
                    if (item.lang !== undefined) {
                        const langElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', 'v8:lang');
                        langElement.textContent = String(item.lang);
                        childElement.appendChild(langElement);
                    }
                    if (item.content !== undefined) {
                        const contentElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', 'v8:content');
                ...
                const itemElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', 'v8:item');
                itemElement.textContent = String(item);
                element.appendChild(itemElement);
            }
        }
        ...
        if (('_xsiNil' in value && value._xsiNil === true) ||
            ('nil' in value && ((value.nil === true) || (value.nil === 'true')) && Object.keys(value).length === 1)) {
        ...
        for (const [childKey, childValue] of Object.entries(value)) {
            ...
            if (childKey === 'xsi:nil') {
                element.setAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'xsi:nil', String(childValue));
                continue;
            }
            if (childKey === 'xsi:type') {
                element.setAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'xsi:type', String(childValue));
                continue;
            }
            ...
            if (childKey === 'from') {
                const ln = element.localName || element.tagName;
            ...
            const elementLocalName = element.localName || element.tagName;
            const elementNs = element.namespaceURI || element.namespaceURI;
            ...
            if (elementLocalName === 'InputByString' && childKey === 'Field') {
                ...
                for (const it of items) {
                    ...
                    if (typeof it === 'string') {
                        ...
                        fieldEl.textContent = String(it['#text'] ?? it.text ?? '');
                    }
                    else {
            ...
            if (childKey === 'Link') {
                ...
                const linkElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.3/xcf/readable', 'xr:Link');
                ...
                if (typeof childValue === 'object' && childValue !== null && !Array.isArray(childValue)) {
                    if ('OneOf' in childValue || 'TypeSet' in childValue || childValue.Type === 'OneOf') {
                        // Старая структура OneOf/TypeSet - преобразуем в новую
                        let typesArray = [];
                        if (childValue.Type === 'OneOf' && 'TypeSet' in childValue) {
                            // Структура: { Type: "OneOf", TypeSet: [...] }
                            const typeSet = childValue.TypeSet;
                            if (Array.isArray(typeSet)) {
                                typesArray = typeSet.map((item) => {
                                    if (typeof item === 'object' && item !== null) {
                        ...
                        else if ('OneOf' in childValue && typeof childValue.OneOf === 'object') {
                            // Структура: { OneOf: { Type: [...] } }
                            const oneOfTypes = childValue.OneOf.Type;
                            if (Array.isArray(oneOfTypes)) {
                                typesArray = oneOfTypes.map((item) => {
                                    if (typeof item === 'object' && item !== null) {
                                        if ('Type' in item || 'v8:Type' in item) {
                                            const typeStr = item.Type || item['v8:Type'];
                                            const cleanType = typeof typeStr === 'object' && typeStr !== null && '#text' in typeStr
                        ...
                        for (const typeItem of typesArray) {
                            const v8TypeElement = typeElement.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', 'v8:Type');
                ...
                if (Array.isArray(childValue)) {
                    ...
                    for (const typeItem of childValue) {
                        if (typeof typeItem === 'object' && typeItem !== null) {
                            ...
                            for (const [qualKey, qualValue] of Object.entries(typeItem)) {
                                ...
                                const qualElement = typeElement.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', `v8:${cleanQualKey}`);
                                if (typeof qualValue === 'object' && qualValue !== null && !Array.isArray(qualValue)) {
                                    ...
                                    for (const [nestedKey, nestedValue] of Object.entries(qualValue)) {
                                        ...
                                        const nestedElement = qualElement.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', `v8:${cleanNestedKey}`);
                            ...
                            // Простая строка
                            const v8Element = typeElement.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', 'v8:Type');
                    ...
                    if ('TypeSet' in childValue || 'v8:TypeSet' in childValue) {
                        const typeSetValue = childValue.TypeSet || childValue['v8:TypeSet'];
                        const typeSetElement = typeElement.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', 'v8:TypeSet');
                        if (typeof typeSetValue === 'object' && typeSetValue !== null && '#text' in typeSetValue) {
                            typeSetElement.textContent = String(typeSetValue['#text']);
                        }
                        else {
                    ...
                    if ('Type' in childValue) {
                        const typeValue = childValue.Type;
                    ...
                    if (!('Type' in childValue && Array.isArray(childValue.Type))) {
                        for (const [typeKey, typeValue] of Object.entries(childValue)) {
                            ...
                            const v8Element = typeElement.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', `v8:${typeKey}`);
                            if (typeof typeValue === 'object' && typeValue !== null && !Array.isArray(typeValue)) {
                                ...
                                for (const [nestedKey, nestedValue] of Object.entries(typeValue)) {
                                    const nestedElement = v8Element.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', `v8:${nestedKey}`);
                                    if (typeof nestedValue === 'object' && nestedValue !== null && !Array.isArray(nestedValue)) {
                                        ...
                                        for (const [deepKey, deepValue] of Object.entries(nestedValue)) {
                                            const deepElement = nestedElement.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', `v8:${deepKey}`);
                ...
                const v8Element = element.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', `v8:${cleanKey}`);
                if (typeof childValue === 'object' && childValue !== null && '#text' in childValue) {
                    v8Element.textContent = String(childValue['#text']);
                }
                else {
                    v8Element.textContent = String(childValue);
                }
                ...
                if (typeof childValue === 'object' && childValue !== null) {
                    ...
                    if ('lang' in cv || 'content' in cv || 'v8:lang' in cv || 'v8:content' in cv) {
                        const v8Item = element.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', 'v8:item');
                ...
                if (typeof childValue === 'object' && childValue !== null) {
                    if (Array.isArray(childValue)) {
                        ...
                        for (const item of childValue) {
                            const itemElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.2/managed-application/core', 'app:item');
                            if (item && typeof item === 'object' && 'name' in item) {
                                itemElement.setAttribute('name', String(item.name));
                            }
                            if (item && typeof item === 'object' && 'value' in item) {
                                const valueElement = itemElement.ownerDocument.createElementNS('http://v8.1c.ru/8.2/managed-application/core', 'app:value');
                                const val = item.value;
                                if (typeof val === 'object' && val !== null && 'text' in val && 'type' in val) {
                                    valueElement.setAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'xsi:type', String(val.type));
            ...
            if (childKey.startsWith('xr:')) {
                childElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.3/xcf/readable', childKey);
            }
            else if (childKey.startsWith('app:')) {
                childElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.2/managed-application/core', childKey);
            }
            else if (childKey.startsWith('v8:')) {
                childElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', childKey);
            }
            else {
 ...
 */
function setComplexXmlStructure(element, value, defaultNamespace) {
    if (typeof value === 'string') {
        element.textContent = value;
    }
    else if (typeof value === 'object' && value !== null) {
        if (Array.isArray(value)) {
            ...
            for (const [key, val] of Object.entries(value)) {
                ...
                if (key.startsWith('xr:')) {
                    childElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.3/xcf/readable', key);
                }
                else if (key.startsWith('app:')) {
                    childElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.2/managed-application/core', key);
                }
                else if (key.startsWith('v8:')) {
                    childElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', key);
                }
                else if (defaultNamespace === 'xr') {
                    childElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.3/xcf/readable', `xr:${key}`);
                }
                else if (defaultNamespace === 'app') {
                    childElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.2/managed-application/core', `app:${key}`);
                }
                else if (defaultNamespace === 'v8') {
                    childElement = element.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', `v8:${key}`);
                }
                else {
                ...
                if (typeof val === 'object' && val !== null && !Array.isArray(val)) {
                    if ('text' in val && 'type' in val) {
                        ...
                        if (valType) {
                            childElement.setAttributeNS('http://www.w3.org/2001/XMLSchema-instance', 'xsi:type', String(valType));
                        }
 ...
 */
function applyChildObjectsChangesWithDom(objElement, attributes, tabularSections, accountingFlags, extDimensionAccountingFlags) {
    ...
    function getNameFromProperties(containerEl) {
    ...
    function syncAndApplyTopLevel(tagName, items) {
        const keepUuids = new Set();
        const keepNames = new Set();
        for (const a of items) {
            if (!a)
                continue;
            if (a.uuid)
                keepUuids.add(String(a.uuid));
            if (a.name)
                keepNames.add(String(a.name));
        }
        ...
        for (const el of existingEls) {
            ...
            const shouldKeep = (uuid && keepUuids.size > 0 && keepUuids.has(uuid)) ||
    ...
    if (attributes !== undefined) {
        ...
        const attrs = all.filter((a) => !a?.childObjectKind || a.childObjectKind === 'Attribute');
        const resources = all.filter((a) => a?.childObjectKind === 'Resource');
        const dimensions = all.filter((a) => a?.childObjectKind === 'Dimension');
    ...
    if (tabularSections !== undefined) {
        ...
        for (const t of tabularSections) {
            if (!t)
                continue;
            if (t.uuid)
                keepUuids.add(String(t.uuid));
            if (t.name)
                keepNames.add(String(t.name));
        }
 ...
 */
function applyNamedChildObjectChangesWithDom(childObjectsElement, item, tagName) {
    if (!item?.name)
        return;
    ...
    if (!element) {
        ...
        element.setAttribute('uuid', item.uuid ? String(item.uuid) : (0, crypto_1.randomUUID)());
...
function applyChildObjectItemChangesWithDom(childObjectsElement, item, tagName) {
...
function applyAttributeChangesWithDom(childObjectsElement, attr) {
    if (!attr.name)
        return;
    ...
    if (!attributeElement) {
        ...
        attributeElement.setAttribute('uuid', attr.uuid ? String(attr.uuid) : (0, crypto_1.randomUUID)());
        ...
        for (let i = 0; i < childObjectsElement.childNodes.length; i++) {
            ...
            if (n.nodeType === 1 && n.tagName === 'Form') {
                insertBeforeNode = n;
                break;
            }
        }
    ...
    if (attr.properties) {
        for (const [key, value] of Object.entries(attr.properties)) {
                    ...
                    if (('OneOf' in valueAny) || ('TypeSet' in valueAny) || (valueAny.Type === 'OneOf')) {
                        ...
                        if (!Array.isArray(typeValue)) {
                            for (const [typeKey, typeVal] of Object.entries(value)) {
                                ...
                                const v8Element = typeElement.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', `v8:${cleanKey}`);
                                if (typeof typeVal === 'object' && typeVal !== null && !Array.isArray(typeVal)) {
                                    for (const [nestedKey, nestedValue] of Object.entries(typeVal)) {
                                        const cleanNestedKey = nestedKey.startsWith('v8:') ? nestedKey.substring(3) : nestedKey;
                                        const nestedElement = v8Element.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', `v8:${cleanNestedKey}`);
                                        nestedElement.textContent = String(nestedValue);
                                        v8Element.appendChild(nestedElement);
                                    }
                                }
            ...
            if (key === 'TypeSet' || key === 'v8:TypeSet') {
                ...
                if (!typeSetElement) {
                    typeSetElement = propertiesElement.ownerDocument.createElementNS('http://v8.1c.ru/8.1/data/core', 'v8:TypeSet');
                ...
                if (typeof value === 'object' && value !== null && '#text' in value) {
                    typeSetElement.textContent = String(value['#text']);
                }
                else {
 ...
 */
function applyTabularSectionChangesWithDom(childObjectsElement, ts) {
    if (!ts.name)
        return;
    ...
    if (!tsElement) {
        ...
        tsElement.setAttribute('uuid', ts.uuid ? String(ts.uuid) : (0, crypto_1.randomUUID)());
        ...
        let parentElement = childObjectsElement.parentElement;
        ...
        let searchElement = childObjectsElement.parentElement;
        while (searchElement) {
        ...
        const typeId1 = (0, crypto_1.randomUUID)();
        const valueId1 = (0, crypto_1.randomUUID)();
        const typeId2 = (0, crypto_1.randomUUID)();
        const valueId2 = (0, crypto_1.randomUUID)();
        // Создаем xr:GeneratedType для TabularSection
        const tabularSectionTypeElement = tsElement.ownerDocument.createElementNS('http://v8.1c.ru/8.3/xcf/readable', 'xr:GeneratedType');
        ...
        const typeIdElement = tsElement.ownerDocument.createElementNS('http://v8.1c.ru/8.3/xcf/readable', 'xr:TypeId');
        typeIdElement.textContent = typeId1;
        tabularSectionTypeElement.appendChild(typeIdElement);
        const valueIdElement1 = tsElement.ownerDocument.createElementNS('http://v8.1c.ru/8.3/xcf/readable', 'xr:ValueId');
        ...
        const tabularSectionRowTypeElement = tsElement.ownerDocument.createElementNS('http://v8.1c.ru/8.3/xcf/readable', 'xr:GeneratedType');
        ...
        const rowTypeIdElement = tsElement.ownerDocument.createElementNS('http://v8.1c.ru/8.3/xcf/readable', 'xr:TypeId');
        rowTypeIdElement.textContent = typeId2;
        tabularSectionRowTypeElement.appendChild(rowTypeIdElement);
        const rowValueIdElement = tsElement.ownerDocument.createElementNS('http://v8.1c.ru/8.3/xcf/readable', 'xr:ValueId');
    ...
    if (ts.attributes && Array.isArray(ts.attributes)) {
        ...
        let tsChildObjectsElement = null;
        for (let i = 0; i < tsElement.childNodes.length; i++) {
            const child = tsElement.childNodes[i];
            if (child.nodeType === 1 && child.tagName === 'ChildObjects') {
                tsChildObjectsElement = child;
                break;
            }
        }
        if (!tsChildObjectsElement) {
            ...
            for (let i = 0; i < tsElement.childNodes.length; i++) {
                const child = tsElement.childNodes[i];
                if (child.nodeType === 1 && child.tagName === 'Properties') {
                    insertAfter = child;
                    break;
                }
            }
        ...
        for (let i = 0; i < tsChildObjectsElement.childNodes.length; i++) {
            ...
            if (n.nodeType === 1 && n.tagName === 'Attribute') {
                existingAttrEls.push(n);
            }
        }
 ...
 * @returns обновленная XML строка
 */
function removeButtonsWithMissingCommands(formElement, allowedCommandNames) {
    ...
    if (!allowedCommandNames)
        return;
    const extractFormCommandName = (raw) => {
    ...
    const buttons = Array.from(formElement.getElementsByTagName?.('Button') || []);
    for (const btn of buttons) {
        const cmdEl = getDirectElementChildren(btn).find((c) => c.tagName === 'CommandName') || null;
        if (!cmdEl)
            continue;
        const cmdName = extractFormCommandName(cmdEl.textContent);
        if (!cmdName)
            continue; // удаляем только ссылки на Form.Command.*
...
function applyFormChangesToXmlStringWithDom(originalXml, formData) {
    ...
    if (Array.isArray(formData.commands)) {
        for (const c of formData.commands) {
            if (c && c.name)
                allowedCommandNames.add(String(c.name));
        }
    }
...
exports.applyFormChangesToXmlStringWithDom = applyFormChangesToXmlStringWithDom;
/**
 ...
 */
function applyFormRootPropertiesChangesWithDom(formElement, properties) {
    const directChildren = Array.from(formElement.childNodes || []).filter((n) => n && n.nodeType === 1);
    for (const [key, value] of Object.entries(properties || {})) {
        if (value === undefined || value === null)
            continue;
        ...
        const propEl = directChildren.find((el) => el.tagName === key) || null;
        if (!propEl)
            continue;
 ...
 */
function applyFormAttributesChangesWithDom(formElement, attributes) {
    // Ищем только прямого ребёнка <Attributes>, чтобы не задеть вложенные структуры
    let attributesElement = null;
    const directChildren = Array.from(formElement.childNodes || []).filter((n) => n && n.nodeType === 1);
    attributesElement = directChildren.find((el) => el.tagName === 'Attributes') || null;
    if (!attributesElement) {
        attributesElement = formElement.ownerDocument.createElement('Attributes');
        formElement.appendChild(attributesElement);
    }
    const existingAttrEls = Array.from(attributesElement.childNodes || [])
        .filter((n) => n && n.nodeType === 1 && n.tagName === 'Attribute');
    const getExistingName = (el) => {
        const a = el.getAttribute('name');
        if (a)
            return String(a);
        const nameEl = el.getElementsByTagName ? el.getElementsByTagName('Name')[0] : null;
    ...
    for (const a of attributes) {
        if (a && a.name)
            desiredNames.add(String(a.name));
    }
    ...
    for (const attr of attributes) {
        ...
        for (const [k, v] of Object.entries(props)) {
            ...
            const existing = propChildren.find((el) => el.tagName === k) || null;
            if (!existing) {
                if (!created)
                    continue;
 ...
 */
function applyFormCommandsChangesWithDom(formElement, commands) {
    // Ищем только прямого ребёнка <Commands>
    let commandsElement = null;
    const directChildren = Array.from(formElement.childNodes || []).filter((n) => n && n.nodeType === 1);
    commandsElement = directChildren.find((el) => el.tagName === 'Commands') || null;
    if (!commandsElement) {
        commandsElement = formElement.ownerDocument.createElement('Commands');
        formElement.appendChild(commandsElement);
    }
    const existingCmdEls = Array.from(commandsElement.childNodes || [])
        .filter((n) => n && n.nodeType === 1 && n.tagName === 'Command');
    const getExistingName = (el) => {
        const a = el.getAttribute('name');
        if (a)
            return String(a);
    ...
    for (const c of commands) {
        if (c && c.name)
            desiredNames.add(String(c.name));
    }
 ...
 */
function childItemKey(tagName, name, id) {
    const n = String(name || '').trim();
    const i = String(id || '').trim();
    return `${tagName}::${n}::${i}`;
}
function applyFormAttributeTypeWithDom(attrEl, attr, created) {
    // Консервативно: тип задаём только при создании нового реквизита.
    if (!created)
        return;
    const typeValue = attr?.type ?? null;
    if (!typeValue)
        return;
    const existingType = getDirectElementChildren(attrEl).find((el) => el.tagName === 'Type') || null;
    if (existingType) {
...
function applyUseAlwaysWithDom(ownerEl, value, allowCreate) {
    if (value === undefined || value === null)
        return;
    const t = typeof value;
    if (t !== 'string' && t !== 'number' && t !== 'boolean')
    ...
    const useAlwaysEl = getDirectElementChildren(ownerEl).find((el) => el.tagName === 'UseAlways') || null;
    if (!useAlwaysEl) {
        if (!allowCreate)
            return;
    ...
    const fieldEl = getDirectElementChildren(useAlwaysEl).find((el) => el.tagName === 'Field') || null;
    if (fieldEl) {
...
function getDirectElementChildren(parent) {
    return Array.from(parent.childNodes || []).filter((n) => n && n.nodeType === 1);
}
function findDirectChildItemsElement(parent) {
    const kids = getDirectElementChildren(parent);
    return kids.find((k) => k.tagName === 'ChildItems') || null;
}
function ensureDirectChildItemsElement(parent) {
...
function ensureFormItemElement(parentChildItemsEl, item) {
    const kids = getDirectElementChildren(parentChildItemsEl);
    const candidates = kids.filter((k) => k.tagName === item.type);
    for (const c of candidates) {
        const nameAttr = c.getAttribute('name');
        const idAttr = c.getAttribute('id');
        if (item.name && nameAttr === item.name)
            return c;
        if (!item.name && item.id && idAttr === String(item.id))
            return c;
        if (item.name && item.id && nameAttr === item.name && idAttr === String(item.id))
            return c;
    }
    // create
    const el = parentChildItemsEl.ownerDocument.createElement(item.type);
    if (item.name)
        el.setAttribute('name', String(item.name));
    if (item.id)
        el.setAttribute('id', String(item.id));
    // вставляем в конец (минимум сдвигов индексов)
    parentChildItemsEl.appendChild(el);
    return el;
}
function applyFormItemCreateAllowedWithDom(itemElement, item) {
    if (!itemElement || !item)
        return;
    // свойства
    const props = item.properties || {};
    for (const [key, value] of Object.entries(props)) {
        ...
        if (key === 'Title' || key === 'Caption') {
            ...
            let titleEl = kids.find((k) => k.tagName === key) || null;
            if (!titleEl) {
            ...
            let v8Item = titleKids.find((k) => k.tagName === 'v8:item') || null;
            if (!v8Item) {
        ...
        if (key === 'Events') {
            ...
            const kids = getDirectElementChildren(itemElement);
            let evEl = kids.find((k) => k.tagName === 'Events') || null;
            if (!evEl) {
        ...
        if (key === 'ContextMenu' || key === 'ExtendedTooltip') {
            const kids = getDirectElementChildren(itemElement);
            let cm = kids.find((k) => k.tagName === key) || null;
            if (!cm) {
        ...
        if (t !== 'string' && t !== 'number' && t !== 'boolean')
            continue;
        const kids = getDirectElementChildren(itemElement);
        let propEl = kids.find((k) => k.tagName === key) || null;
        if (!propEl) {
...
function applyFormChildItemsListWithDom(parentChildItemsEl, desiredItems, allowedCommandNames) {
    const desiredRaw = Array.isArray(desiredItems) ? desiredItems : [];
    const shouldIgnoreType = (type) => {
        const t = String(type || '').trim();
        if (!t)
            return true;
        if (t.startsWith('#'))
            return true; // preserveOrder '#text'
        // Эти узлы не являются элементами дизайнера в <ChildItems>
        if (t === 'Unknown' || t === 'ContextMenu' || t === 'AutoCommandBar')
            return true;
        return false;
    };
    const extractFormCommandName = (raw) => {
    ...
    const referencesMissingFormCommand = (it) => {
        if (!allowedCommandNames)
            return false;
        const cmd = it?.properties ? it.properties?.CommandName : undefined;
    ...
    const desiredKeys = new Set(desired.map((it) => childItemKey(it.type, it.name || null, it.id ? String(it.id) : null)));
    for (const el of existingKids) {
        if (allowedCommandNames) {
            const cmdEl = getDirectElementChildren(el).find((c) => c.tagName === 'CommandName') || null;
    ...
    for (const it of desired) {
        ...
        for (const el of kidsNow) {
            const k = childItemKey(el.tagName, el.getAttribute('name'), el.getAttribute('id'));
            if (k === key) {
                target = el;
                break;
            }
...
function applyFormChildItemsChangesWithDom(formElement, childItems, allowedCommandNames) {
 ...
 */
function applyMultilingualTitleLikeWithDom(itemElement, tagName, value) {
    // Найдём прямого ребёнка <Title>/<Caption>
    const directChildren = Array.from(itemElement.childNodes || []).filter((n) => n && n.nodeType === 1);
    const titleEl = directChildren.find((el) => el.tagName === tagName) || null;
    if (!titleEl)
        return;
    ...
    const pushPair = (p) => {
        ...
        const out = {};
        if (lang !== undefined)
            out.lang = String(lang);
        if (content !== undefined)
            out.content = String(content);
        if (out.lang !== undefined || out.content !== undefined)
            pairs.push(out);
    };
    if (typeof value === 'string') {
        ...
        const it = value.item || value['v8:item'] || value.items;
        if (Array.isArray(it)) {
            ...
            if (pairs.length === 0) {
                for (let i = 0; i < it.length; i += 2) {
                    ...
                    if (a && typeof a === 'object') {
                        if ('lang' in a)
                            p.lang = String(a.lang);
                        if ('v8:lang' in a)
                            p.lang = String(a['v8:lang']);
                        if ('content' in a)
                            p.content = String(a.content);
                        if ('v8:content' in a)
                            p.content = String(a['v8:content']);
                    }
                    if (b && typeof b === 'object') {
                        if (p.lang === undefined && ('lang' in b || 'v8:lang' in b))
                            p.lang = String(b.lang ?? b['v8:lang']);
                        if (p.content === undefined && ('content' in b || 'v8:content' in b))
                            p.content = String(b.content ?? b['v8:content']);
                    }
                    if (p.lang !== undefined || p.content !== undefined)
                        pairs.push(p);
                }
            }
        }
    ...
    const items = Array.from(titleEl.childNodes || []).filter((n) => n && n.nodeType === 1);
    const v8Items = items.filter((el) => (el.tagName === 'v8:item' || el.localName === 'item'));
    const getDirectChildText = (parent, wantedTag) => {
        const kids = Array.from(parent.childNodes || []).filter((n) => n && n.nodeType === 1);
        const el = kids.find((k) => k.tagName === wantedTag) || null;
        return el ? (el.textContent ?? '') : '';
    };
    const findDirectChild = (parent, wantedTag) => {
        const kids = Array.from(parent.childNodes || []).filter((n) => n && n.nodeType === 1);
        return kids.find((k) => k.tagName === wantedTag) || null;
    };
    const byLang = new Map();
    for (const itEl of v8Items) {
        const lang = String(getDirectChildText(itEl, 'v8:lang') || '').trim().toLowerCase();
        if (lang)
            byLang.set(lang, itEl);
    }
    ...
    for (const p of pairs) {
        ...
        if (p.content !== undefined) {
            ...
            if (contentEl)
                contentEl.textContent = String(p.content);
        }
    }
}
/**
 ...
 */
function applyEventsWithDom(itemElement, value) {
    const directChildren = Array.from(itemElement.childNodes || []).filter((n) => n && n.nodeType === 1);
    const eventsEl = directChildren.find((el) => el.tagName === 'Events') || null;
    if (!eventsEl)
        return;
    const desired = [];
    ...
    const parseOne = (v) => {
        ...
        if (typeof v !== 'object')
            return;
        const at = v['@'] || v[':@'] || v['@_'] || null;
        const name = v.name ?? (at ? (at.name ?? at['@_name']) : undefined);
        const handler = v.Event ?? v.handler ?? v.text;
        if (name !== undefined) {
    ...
    if (desired.length === 0)
        return;
    const eventNodes = Array.from(eventsEl.childNodes || []).filter((n) => n && n.nodeType === 1);
    const eventEls = eventNodes.filter((el) => el.tagName === 'Event');
    const byName = new Map();
 ...
 */
function applyEmptyElementAttrsWithDom(itemElement, tagName, value) {
    const directChildren = Array.from(itemElement.childNodes || []).filter((n) => n && n.nodeType === 1);
    const el = directChildren.find((c) => c.tagName === tagName) || null;
    if (!el)
        return;
    const pick = (v) => {
    ...
    if (!attrs && Array.isArray(value) && value.length > 0)
        attrs = pick(value[0]);
    if (!attrs)
        return;
...
function applyFormItemChangesWithDom(parentElement, item, allowedCommandNames) {
    if (!item.type)
        return;
    const directChildren = Array.from(parentElement.childNodes || []).filter((n) => n && n.nodeType === 1);
    const candidates = directChildren.filter((el) => el.tagName === item.type);
    ...
    if (item.properties) {
        for (const [key, value] of Object.entries(item.properties)) {
            ...
            const propChildren = Array.from(itemElement.childNodes || []).filter((n) => n && n.nodeType === 1);
            const propElement = propChildren.find((el) => el.tagName === key) || null;
            if (!propElement) {
                continue;
            }
    ...
    if (item.childItems && item.childItems.length > 0) {
        const childItemsElement = Array.from(itemElement.childNodes || []).find((n) => n && n.nodeType === 1 && n.tagName === 'ChildItems');
        if (!childItemsElement)
            return;
...
//# sourceMappingURL=xmlDomUtils.js.map