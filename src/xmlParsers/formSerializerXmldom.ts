 ...
 */
function createElementWithNamespace(doc: Document, tagName: string): Element {
 ...
 */
function setNamespaceAttributeIfNeeded(element: Element, tagName: string): void {
  const namespaceMatch = tagName.match(/^([^:]+):(.+)$/);
  if (namespaceMatch) {
    const prefix = namespaceMatch[1];
    const namespaceURI = getNamespaceURI(prefix);
 ...
 */
export function serializeFormDomToXml(doc: Document): string {
  // ВАЖНО: Удаляем все xmlns:v8 атрибуты из дочерних элементов перед сериализацией
  // Namespace должен быть объявлен только в корневом элементе Form
  function removeNamespaceAttributes(element: Element): void {
    ...
    for (let i = 0; i < element.childNodes.length; i++) {
      ...
      if (child.nodeType === 1) { // ELEMENT_NODE
        removeNamespaceAttributes(child as Element);
      }
    }
  }
 ...
 */
export function updateFormDomFromData(
  doc: Document,
  formElement: Element,
  formData: {
    properties?: Record<string, any>;
  ...
  for (const tagName of elementsToPreserve) {
    ...
    if (existing) {
      preservedElements.set(tagName, existing.cloneNode(true) as Element);
    }
  }
    ...
    formData.properties && typeof formData.properties === 'object' ? formData.properties : {};
  {
    ...
    for (const [key, value] of Object.entries(properties)) {
      ...
      const propElement = existingElement
        ? updatePropertyElement(doc, existingElement.cloneNode(true) as Element, key, value)
  ...
  if (formData.childItems !== undefined) {
      ...
      const childItemsElement = existingChildItems
        ? updateChildItemsElement(doc, existingChildItems.cloneNode(true) as Element, formData.childItems, 1)
  ...
  if (formData.commands) {
    ...
    const commandsElement = existingCommands
      ? updateCommandsElement(doc, existingCommands.cloneNode(true) as Element, formData.commands)
 ...
 */
function updatePropertyElement(doc: Document, existingElement: Element, propName: string, propValue: any): Element | null {
 ...
 */
function createPropertyElement(doc: Document, propName: string, propValue: any): Element | null {
  if (propValue === undefined || propValue === null) return null;
 ...
 */
function updateChildItemsElement(doc: Document, existingElement: Element, childItems: FormItem[], depth: number = 0): Element | null {
 ...
 */
function createChildItemsElement(doc: Document, childItems: FormItem[], depth: number = 0): Element | null {
  if (!childItems || childItems.length === 0) return null;
 ...
 */
function updateCommandsElement(doc: Document, existingElement: Element, commands: FormCommand[]): Element | null {
 ...
 */
function createCommandsElement(doc: Document, commands: FormCommand[]): Element | null {
  if (!commands || commands.length === 0) return null;
    ...
    if (hasProperties) {
      ...
      for (const [key, value] of Object.entries(cmdProps)) {
        ...
        if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') {
          ...
          if (key === 'Title' && typeof value === 'object' && (value as any)['v8:item']) {
            const v8Item = (value as any)['v8:item'];
 ...
 */
function updateFormProperties(doc: Document, formElement: Element, properties: Record<string, any>): void {
 ...
 */
function updateFormAttributes(doc: Document, formElement: Element, attributes: FormAttribute[]): void {
  // Находим или создаем элемент Attributes
  let attributesElement = Array.from(formElement.childNodes).find(
    (n): n is Element => n.nodeType === 1 && (n as Element).tagName === 'Attributes'
  );
  ...
  if (!attributesElement) {
    ...
    const childItemsElement = Array.from(formElement.childNodes).find(
    ...
    for (let i = 0; i < attributesElement.childNodes.length; i++) {
      ...
      if (child.nodeType === 1 && (child as Element).tagName === 'Attribute') {
 ...
 */
function createTypeElement(doc: Document, typeValue: any): Element | null {
 ...
 */
function updateFormCommands(doc: Document, formElement: Element, commands: FormCommand[]): void {
  // Находим или создаем элемент Commands
  let commandsElement = Array.from(formElement.childNodes).find(
    (n): n is Element => n.nodeType === 1 && (n as Element).tagName === 'Commands'
  );
  ...
  if (!commandsElement) {
    ...
    for (let i = 0; i < commandsElement.childNodes.length; i++) {
      ...
      if (child.nodeType === 1 && (child as Element).tagName === 'Command') {
        toRemove.push(child as Element);
      }
    }
 ...
 */
function updateFormChildItems(doc: Document, formElement: Element, childItems: FormItem[]): void {
  // Находим или создаем элемент ChildItems
  let childItemsElement = Array.from(formElement.childNodes).find(
    (n): n is Element => n.nodeType === 1 && (n as Element).tagName === 'ChildItems'
  );
 ...
 */
function createFormItemElement(doc: Document, item: FormItem, depth: number = 0): Element | null {
 ...
 * @returns XML строка
 */
export function serializeFormToXml(formData: ParsedFormFullXmldom): string {
  ...
  const newDoc = formData._domDocument.cloneNode(true) as Document;
